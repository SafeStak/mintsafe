...
public class GenericMintingService
{
    private readonly ILogger<GenericMintingService> _logger;
    ...
    public GenericMintingService(
        ILogger<GenericMintingService> logger,
    ...
    public async Task<BuiltTransaction?> MintNativeAssets(
        string from, 
        string to,
        Network network,
        Balance? balanceToSend = null, 
        bool sendAll = false,
        Dictionary<NativeAssetValue, Dictionary<string, object>>? nativeAssets = null, 
        string? fromSigningKey = null,
        CancellationToken ct = default)
    {
        ...
        if (protocolParams is null) return null;
        var sourceAddressInfo = (await addressClient.GetAddressInformation(from)).Content?.FirstOrDefault();
        if (sourceAddressInfo is null) return null;
        if (sourceAddressInfo.UtxoSets is null || !sourceAddressInfo.UtxoSets.Any()) return null;
        ...
        var outputBalance = sendAll ? consolidatedInputValue : balanceToSend ?? throw new Exception("Must have a balance to send or send-all");
        var changeBalance = consolidatedInputValue.Subtract(outputBalance);
        var txOutput = sendAll || changeBalance.IsZero()
            ? new[] { new PendingTransactionOutput(to, outputBalance) }
            : new[] {
                new PendingTransactionOutput(to, outputBalance),
        ...
        {
            //sw.Restart();
            //using var stream = new MemoryStream(builtTx.CborBytes);
